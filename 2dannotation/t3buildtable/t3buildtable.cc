...
// Created On       : Sat May 1 14:31:00 2010
...
T3BuildTable::T3BuildTable(int argc, char * argv [])
{
	...
	case 1:
		{
			T3BuildTableH1 tableBuilder(mstrPDBGroupsFile, mstrInteractionsFile);
			tableBuilder.computeTable();
		}
		break;
	case 2:
		{
			T3TableBuilderH2 tableBuilder(mstrPDBGroupsFile, mstrInteractionsFile);
			tableBuilder.computeTable();
		}
		break;
	case 3:
		{
			T3TableBuilderH3 tableBuilder(mstrPDBGroupsFile, mstrInteractionsFile);
			tableBuilder.computeTable();
		}
		break;
	default:
...
void T3BuildTable::version () const
{
...
void T3BuildTable::help () const
{
	mccore::gOut (0)
		<< "This program computes a table of frequency of interaction positions in cycle pairs." << std::endl
...
void T3BuildTable::readOptions (int argc, char* argv[])
{
	int c;
