...
namespace annotate
{
		...
		std::list<Cycle>& aCycles) const
	{
		...
		if(NULL != pAnnTPairs && NULL != pAnnTStacks)
		{
			...
			while(it != aCycles.end())
			{
				...
				unsigned int uiSize = it->residues().size();
	...
	void AnnotationTertiaryCycles::updateCycleLoops(const AnnotateModel& aModel)
	{
		...
		if(NULL != pAnnLoops)
		{
			...
			{
//				const Loop::interactions_set interactions;
//				interactions = itLoop->getInteractions();
				
				// A cycle requires at least 3 interactions
				if(2 < itLoop->getInteractions().size()) 
				{
					Cycle cycle(aModel, itLoop->getInteractions(), ucRelMask);
					mLoops.insert(cycle);
				}
